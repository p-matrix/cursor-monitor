// =============================================================================
// @pmatrix/cursor-monitor — state-store.ts
// File-based session state persistence
//
// 기반: @pmatrix/claude-code-monitor state-store.ts
// 변경: PersistedSessionState에 Cursor 전용 메타데이터 + 카운터 추가
//
// Each Cursor hook invocation is a new process; in-memory state does not
...
export function loadOrCreateState(sessionId: string, agentId: string): PersistedSessionState {
  ...
  state.framework ??= 'cursor';
  // Backfill guard — Cursor 전용 메타데이터
  state.conversationId ??= '';
  state.cursorVersion ??= '';
  state.workspaceRoot ??= '';
  state.composerMode ??= 'agent';
  state.isBackgroundAgent ??= false;
  state.model ??= '';
  // Backfill guard — Cursor 전용 카운터
  state.toolCallCount ??= 0;
  state.shellCommandCount ??= 0;
  state.shellDenyCount ??= 0;
  state.mcpCallCount ??= 0;
  state.fileEditCount ??= 0;
 ...
 * @param framework — filter by framework (e.g. 'cursor'). If omitted, returns any framework.
 */
