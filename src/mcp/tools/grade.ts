// =============================================================================
// @pmatrix/cursor-monitor — mcp/tools/grade.ts
...
export async function handleGradeTool(
  ...
  if (!config.agentId) {
    return err('P-MATRIX not configured. Run: pmatrix-cursor setup --agent-id <id>');
  }
  if (!config.apiKey) {
    return err('No API key. Set PMATRIX_API_KEY or run: pmatrix-cursor setup');
  }
