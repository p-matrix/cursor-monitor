// =============================================================================
// @pmatrix/cursor-monitor — mcp/server.ts
...
//
// Started by: pmatrix-cursor mcp
// Transport:  stdio (Cursor MCP protocol)
// =============================================================================
...
const TOOLS: Tool[] = [
  {
    name: 'pmatrix_status',
    description:
      'Show current P-MATRIX safety grade, R(t) risk score, mode, and session counters for this Cursor session.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: {
  ...
  },
  {
    name: 'pmatrix_halt',
    description:
      '⛔ HALT — immediately block all tool execution across all Cursor sessions. Creates ~/.pmatrix/HALT. To resume: rm ~/.pmatrix/HALT',
    inputSchema: {
      type: 'object',
      properties: {
        reason: {
  ...
  setInterval(() => {
    try {
      ...
      if (state.axes) {
        ...
        // Degraded SV: Cursor 3.5 설계상 개별 축 값 미보유
